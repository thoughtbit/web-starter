...
const RouterManager = {
      ...
      const uniqueRoutes = routes.filter((route) => {
        return (
          this._registeredRoutes.findIndex(
            (registeredRoute) => registeredRoute.route.name === route.name && registeredRoute.route.path === route.path
          ) < 0
        );
      });
  ...
  addRoutesByPriority: function(routesData) {
    ...
    router.addRoutes(routesToAdd.map((r) => r.route));
  },
...
const createRouterProxy = (router: VueRouter): VueRouter => {
  return new Proxy(router, {
    get(target, propKey) {
      // @ts-ignore
      const origMethod = target[propKey];
      if (propKey === "addRoutes") {
        return (routes: RouteConfig[], ...args): void => RouterManager.addRoutes(routes, ...args);
      }
      return origMethod;
    }
  });
...
export { RouterManager, createRouterProxy };
