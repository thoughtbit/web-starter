...
export default class MenuView extends Vue {
  ...
  private closeOthersPage(pageKey: any) {
    ...
    // })
    // @ts-ignore
    const dashboardIndex = this.pageList.find((d) => d.name === "dashboard");

    this.linkList = this.linkList.slice(index, index + 1);
    this.pageList = this.pageList.slice(index, index + 1);
    let idx = 0;
    // @ts-ignore
    if (dashboardIndex && !this.pageList.find((d) => d.name === "dashboard")) {
      // @ts-ignore
  ...
  private closeAllPage(pageKey: any) {
    ...
    // @ts-ignore
    this.activePage = workplaceIndex ? workplaceIndex.fullPath : "/dashboard";
  }
