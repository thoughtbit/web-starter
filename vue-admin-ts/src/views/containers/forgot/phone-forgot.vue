<template>
  <el-form ref="forgotForm" :model="forgotForm" :rules="forgotRules" class="forgot-form" label-position="left">
    <div class="el-form-item-tip">
      如果您的账号可通过手机号码登录，请输入短信验证码重设登录密码
    </div>
...
<script lang="ts">
  ...
  @Component
  export default class PhoneForgot extends Vue {
    private timer: any;
    ...
    private handleSendVerifyCode() {
      if (!validMobile(this.forgotForm.phone)) {
        this.$message({
          showClose: true,
          type: "warning",
          message: "请输入正确的手机号码"
        });
        this.$refs.forgotForm.validateField("phone");
        return false;
      }

      SERVICE["getSnsCode"]({ phone: this.forgotForm.phone })
        .then(() => {
