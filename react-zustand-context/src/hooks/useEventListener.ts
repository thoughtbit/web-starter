...
export default function useEventListener<Type extends keyof DocumentEventMap>(
  ...
  { options, enabled = true, dependencies = [enabled, type], element }: UseEventOptions = {}
) {
  const memoizedEventCallback: React.MutableRefObject<(e: DocumentEventMap[Type]) => void> = useRef(() => {});
  ...
  useEffect(() => {
    ...
    (element ?? document).addEventListener(type, callback as EventListener, eventListenerOptions);

    return () => {
      (element ?? document).removeEventListener(type, callback as EventListener, eventListenerOptions);
    };
